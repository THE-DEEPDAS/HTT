...
import { ArrowLeft, Upload, X, Camera, Loader2, CheckCircle, Calendar, Clock, Package, AlertCircle, RotateCcw, Zap, CreditCard } from 'lucide-react';

const Return = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const fileInputRef = useRef(null);
  const { orderDetailId, productId, orderId, returnReason } = location.state || {};
  ...
  if (!orderDetailId) {
    return (
      <div className="page-container text-center py-20">
        ...
        <p className="text-gray-500 mb-4">No item selected for return</p>
  ...
  const submitImage = async () => {
    ...
    try {
      ...
      const response = await api.post('/inference2/', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const processReturn = async () => {
    ...
    setError('');
    try {
      const inferStatus = inferenceResult?.status || 'refurb';
      const response = await api.post('/return/process-pickup/', {
        order_detail_id: orderDetailId,
        inference_status: inferStatus,
        pickup_date: inferStatus === 'scrap' ? null : (pickupDate || null),
        pickup_time: inferStatus === 'scrap' ? null : (pickupTime || null),
        express_delivery: expressDelivery,
      });
      setPickupData(response.data);
      setShowPickupModal(true);
    } catch (err) {
      setError(err.response?.data?.message || err.response?.data?.error || 'Failed to process return. Please try again.');
  ...
  const categoryLabel = (cat) => {
    if (cat === 'scrap') return { label: 'Non-Recoverable', color: 'bg-red-50 text-red-600', icon: '♻️' };
    if (cat === 'refurb') return { label: 'Refurbishable', color: 'bg-amber-50 text-amber-600', icon: '🔧' };
    return { label: 'Resaleable', color: 'bg-emerald-50 text-emerald-600', icon: '✅' };
  };
    ...
    <motion.div className="page-container max-w-2xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      {step === 1 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card p-6">
          ...
          <p className="text-sm text-gray-500 mb-6">Take a clear photo of the product to verify its condition.</p>
      ...
      {/* Step 2: Inference Result & Process */}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div className="card p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-4 space-y-3">
              ...
              {(() => {
                const cat = categoryLabel(inferenceResult.status);
                return (
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">Condition</span>
                    <span className={`badge text-xs ${cat.color}`}>{cat.icon} {cat.label}</span>
                  </div>
                );
              })()}
              <p className="text-sm text-gray-500 pt-2 border-t border-gray-100">
                {inferenceResult.status === 'resale' && 'Product is in excellent condition and eligible for resale.'}
                {inferenceResult.status === 'refurb' && 'Product needs refurbishment but is eligible for return.'}
                {inferenceResult.status === 'scrap' && 'Product is too damaged. Return will proceed without pickup.'}
              </p>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${
                  expressDelivery ? 'bg-amber-100' : 'bg-gray-100'
                }`}>
                  <Zap className={`w-5 h-5 ${expressDelivery ? 'text-amber-600' : 'text-gray-400'}`} />
                </div>
                <div>
                  <p className="text-sm font-bold text-gray-900">Express Pickup</p>
                  <p className="text-xs text-gray-500 mt-0.5">Faster pickup within 24 hours</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className={`w-11 h-6 rounded-full transition-colors flex items-center px-0.5 ${
                  expressDelivery ? 'bg-amber-500' : 'bg-gray-200'
                }`}>
                  <div className={`w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${
                    expressDelivery ? 'translate-x-5' : 'translate-x-0'
                  }`} />
                </div>
              </div>
            </div>
            {expressDelivery && (
              <motion.p
                ...
                className="text-xs text-amber-700 mt-3 bg-amber-100/60 rounded-lg p-2.5"
              >
                A 7% convenience fee will be deducted from your refund amount for express pickup service.
              </motion.p>
            )}
          </div>

          <button onClick={processReturn} disabled={processing || (inferenceResult.status !== 'scrap' && (!pickupDate || !pickupTime))} className="btn-primary w-full disabled:opacity-50">
            {processing ? <><Loader2 className="w-4 h-4 animate-spin" /> Processing...</> : 'Proceed with Return'}
          </button>
        </motion.div>
      )}
      ...
      {step === 3 && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card p-8 text-center">
          ...
          <h2 className="text-xl font-bold text-gray-900 mb-2">Return Confirmed!</h2>
          <p className="text-sm text-gray-500 mb-6">We'll arrange a pickup for your item.</p>

          {pickupData && (
            <div className="bg-gray-50 rounded-xl p-4 text-left space-y-2 mb-6">
              {pickupData.express_delivery && (
                <div className="flex items-center gap-2 text-sm">
                  <Zap className="w-4 h-4 text-amber-500" />
                  <span className="text-amber-700 font-semibold">Express Pickup</span>
                </div>
              )}
              ...
              {pickupData.express_delivery && pickupData.original_refund_amount && (
                <div className="flex items-center gap-2 text-sm">
                  <span className="text-gray-400 line-through ml-6">₹{parseFloat(pickupData.original_refund_amount).toLocaleString()}</span>
                  <span className="text-xs text-amber-600">−7% express fee</span>
                </div>
              )}
              {pickupData.refund_amount && (
                <div className="flex items-center gap-2 text-sm">
                  <CreditCard className="w-4 h-4 text-gray-400" />
                  <span className="text-gray-600">Refund: <strong className="text-emerald-600">₹{parseFloat(pickupData.refund_amount).toLocaleString()}</strong></span>
                </div>
              )}
            </div>
          )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-white rounded-3xl shadow-elevated p-6 w-full max-w-md">
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-lg font-bold text-gray-900">Pickup Scheduled</h3>
                ...
                <div className="space-y-4 mb-6">
                  <div className="bg-emerald-50 rounded-xl p-4">
                    <p className="text-sm font-semibold text-emerald-700">
                      {pickupData.message || 'Your return has been processed successfully.'}
                    </p>
                  </div>
                  {pickupData.express_delivery && (
                    <div className="flex items-center gap-3 p-3 bg-amber-50 rounded-xl border border-amber-200">
                      ...
                      <div>
                        <p className="text-xs text-amber-600 font-bold">Express Pickup</p>
                        <p className="text-xs text-amber-500">7% fee (₹{pickupData.express_delivery_fee}) deducted</p>
                      </div>
                    </div>
                  )}
                  {pickupData.pickup_date && (
